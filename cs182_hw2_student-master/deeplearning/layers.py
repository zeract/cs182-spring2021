...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = gamma * (1.0 / N) * np.power(sample_var + eps, -0.5) * (
                N * dout - dout.sum(axis=0) - ((x - sample_mean) * dout).sum(axis=0) * (x - sample_mean) * np.power(
            sample_var + eps, -1))
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out  = np.dot(w_row,x_col).reshape(N,F,H_prime,W_prime)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b, conv_param)
                ...
                for j in range(WW):
                    for l in range(pool_height):
                        for m in range(pool_width):
                            if dx[n, c, i * stride + l, j * stride + m] == np.max(
                                    dx[n, c, i * stride:i * stride + pool_height, j * stride:j * stride + pool_width]):
                                dx[n, c, i * stride + l, j * stride + m] = dout[n, c, i, j]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #############################################################################
    N, C, H, W = x.shape
    sample_mean = np.sum(x, axis=(0,2,3)) / N
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
